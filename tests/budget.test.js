...
const budgetHeaders = { 'x-api-key': process.env.API_KEY };
...
describe('Budget endpoints', () => {
  it('sets and retrieves a budget for a given month', async () => {
    ...
    const fetched = await request(app).get('/budget/2026-09').set(budgetHeaders);
    expect(fetched.status).toBe(200);
    expect(fetched.body.amount).toBe(500);
  });

  it('returns 404 for a month with no budget set', async () => {
    const res = await request(app).get('/budget/1999-01').set(budgetHeaders);
  ...
  it('flags a month as over budget in the monthly summary', async () => {
    ...
    await request(app).post('/budget').set(budgetHeaders).send({ month: '2026-08', amount: 100 });
    await request(app)
      .post('/expenses')
      .set(headers)
